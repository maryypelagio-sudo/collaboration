<script>
    ...
    function confirmArchiveItem() {
        ...
        router.post(
            `/inventory/items/${selectedItem.id}/archive`,
            {
...
<AuthenticatedLayout>
    <div class="space-y-6 relative">
        ...
        >
            <div>
                ...
                <p class="text-slate-500 mt-1">
                    Manage and track all iReply back office supplies and
                    equipment.
                </p>
            </div>
        ...
        >
            ...
            <select
                class="w-full md:w-48 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            >
            ...
            <button
                class="w-full md:w-auto px-4 py-2 text-slate-600 hover:bg-slate-50 rounded-xl transition-colors font-medium text-sm flex items-center justify-center gap-2"
            >
                <ArrowUpDown size={16} />
                Sort
            </button>
        ...
        <div class="flex flex-wrap gap-2">
            <button
                ...
                class="px-4 py-2 rounded-xl text-sm font-bold transition-all {selectedStatus ===
                'all'
        ...
        <!-- Inventory Table -->
        <div
            class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden z-0"
        >
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 flex-shrink-0 group-hover:bg-white group-hover:shadow-sm transition-all border border-transparent group-hover:border-slate-100"
                                        >
                                            <Package size={20} />
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-semibold border border-slate-200"
                                    >
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span
                                        class="px-3 py-1 border rounded-full text-[10px] font-bold uppercase tracking-wide {getStatusColor(
                                            item.quantity,
                                ...
                                <td class="px-6 py-4 text-center">
                                    {#if item.is_active}
                                        <span
                                            class="px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-[10px] font-bold"
                                            >ACTIVE</span
                                        >
                                    {:else}
                                        <span
                                            class="px-2 py-1 bg-slate-100 text-slate-500 rounded text-[10px] font-bold"
                                            >INACTIVE</span
                                        >
                                    {/if}
                                    {#if item.is_rarely_used}
                                        <span
                                            class="ml-1 px-2 py-1 bg-amber-100 text-amber-700 rounded text-[10px] font-bold"
                                            >RARELY USED</span
                                        >
                                    {/if}
                                </td>
                        ...
                        {:else}
                            <tr>
                                <td colspan="5" class="px-6 py-20 text-center">
                                    ...
                                    >
                                        <div
                                            class="w-12 h-12 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center text-slate-300"
                                        >
                                            <Package size={24} />
                                        </div>
                                        <div>
            ...
            <div
                class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 font-medium"
            >
                <p>Showing {filteredItems.length} of {items.length} items</p>
                <div class="flex items-center gap-2">
                    <button
                        class="px-3 py-1 border border-slate-200 rounded-lg hover:bg-white disabled:opacity-50 transition-all font-bold"
                    ...
                    >
                        <div>
                            ...
                            <input
                                ...
                                class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                                required
                            />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white"
                                    required
                                >
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    id="item_min"
                                    type="number"
                                    bind:value={$itemForm.min_stock_level}
                                    min="0"
                                    class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                                />
                                {#if $itemForm.errors?.min_stock_level}
                                    <p class="text-xs text-rose-500 mt-1">
                                        {$itemForm.errors.min_stock_level}
                                    </p>
                                {/if}
                            </div>
                        </div>
                        ...
                        <div
                            class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 mb-4"
                        >
                            <div class="flex items-center gap-2">
                                <input
                                    type="checkbox"
                                    id="is_active"
                                    bind:checked={$itemForm.is_active}
                        ...
                        >
                            <button
                                type="button"
                                on:click={() => (showItemModal = false)}
                                class="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-50 rounded-xl transition-colors"
                                >Cancel</button
                            >
                            <button
                                type="submit"
                                disabled={$itemForm.processing}
                                class="px-5 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-all disabled:opacity-50"
                            >
                                {$itemForm.processing
                                    ? "Saving..."
                                    : "Save Item"}
    ...
    {#if showCategoryModal}
        ...
        >
            <div
                class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col"
            >
                <div
                    ...
                    >
                        ...
                        >
                            <button
                                type="button"
                                on:click={() => (showCategoryModal = false)}
                                class="px-4 py-2 text-slate-600 font-medium hover:bg-slate-50 rounded-xl transition-colors"
                                >Cancel</button
                            >
                            <button
                                type="submit"
                                disabled={$categoryForm.processing}
                                class="px-4 py-2 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-all disabled:opacity-50"
                            >
                                Save Category
                            </button>
                        </div>
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <button
                        type="button"
                        on:click={() => (showAdjustModal = false)}
                        class="text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-50 transition-colors"
                    >
                        <X size={20} />
                    </button>
                ...
                <div class="p-6">
                    <form
                        on:submit|preventDefault={submitAdjustStock}
                        class="space-y-4"
                    >
                        <div>
                            <label
                        ...
                        <div>
                            ...
                            <input
                                id="adjust_quantity"
                                type="number"
                                bind:value={$adjustForm.quantity}
                                min="0"
                                class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                                required
                            />
                        ...
                        >
                            ...
                            <button
                                ...
                                class="px-4 py-2 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 transition-all disabled:opacity-50"
                            >
                                Apply Changes
                            </button>
                        </div>
            ...
            >
                ...
                <div class="p-6">
                    <p class="text-sm text-slate-600 mb-4">
                        Provide the reason for archiving <strong
                ...
                <div
                    class="flex justify-end gap-3 p-4 border-t border-slate-100"
                >
                    <button
                        type="button"
                        on:click={() => {
                    ...
                    <button
                        type="button"
                        on:click={confirmArchiveItem}
                        class="px-4 py-2 bg-rose-600 text-white font-semibold rounded-xl hover:bg-rose-700 transition-all"
                        >Archive Item</button
                    >
                </div>
            </div>
            ...
            >
                ...
                >
                    ...
                    <button
                        type="button"
                        on:click={() => {
                            showArchivedItemDetailsModal = false;
                            selectedItem = null;
                        }}
                        class="text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-50 transition-colors"
                    >
                ...
                >
                    ...
                    <button
                        type="button"
                        on:click={() => restoreItem(selectedItem.id)}
                        class="px-4 py-2 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 transition-all"
                        >Restore</button
                    >
                    <button
                        type="button"
        ...
        >
            ...
            >
                ...
                <div class="p-6">
                    ...
                    <form
                        on:submit|preventDefault={submitReportDamage}
                        class="space-y-4"
                    >
                        <div>
                            <label
                        ...
                        >
                            ...
                            <button
                                type="submit"
                                disabled={$damageForm.processing}
                                class="px-4 py-2 bg-rose-600 text-white font-semibold rounded-xl hover:bg-rose-700 transition-all disabled:opacity-50"
                            >
                                Report Damage
                            </button>
                        </div>
